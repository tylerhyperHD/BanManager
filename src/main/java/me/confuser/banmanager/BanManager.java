...
public class BanManager extends BukkitPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            PluginLogger.warn("An error occurred attempting to make a database connection, please see stack trace below\n" + e);
        ...
        try {
            ...
            if (timeDiff > 1) {
                PluginLogger.severe("The time on your server and MySQL database are out by " + timeDiff + " seconds, this may cause syncing issues.");
            }
        } catch (SQLException e) {
            PluginLogger.warn("An error occurred attempting to find the time difference, please see stack trace below\n" + e);
            plugin.getPluginLoader().disablePlugin(this);
        }
    ...
    public boolean setupConnections() throws SQLException {
        if (!configuration.getLocalDb().isEnabled()) {
            PluginLogger.warn("Local Database is not enabled, disabling plugin");
            plugin.getPluginLoader().disablePlugin(this);
            return false;
        }
    ...
    private ConnectionSource setupConnection(DatabaseConfig dbConfig) throws SQLException {
        ...
        if (dbConfig.getLeakDetection() != 0) {
            ds.setLeakDetectionThreshold(dbConfig.getLeakDetection());
        }
     ...
     * above.
        ...
        // Purge
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, new Purge());
    }
    ...
    private void setupAsyncRunnable(long length, Runnable runnable) {
        ...
        plugin.getServer().getScheduler().runTaskTimerAsynchronously(plugin, runnable, length, length);
    }
}
